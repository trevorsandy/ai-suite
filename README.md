...
**AI-Suite** extends [Cole Medin's](https://github.com/coleam00) [
...
It provides an open, curated, pre-configured docker compose template
that bootstraps a fully featured Local AI and Low Code development 
environment on a self-hosted n8n platform, enabling users to focus on 
...
✅ [**Neo4j**](https://neo4j.com/) - Knowledge graph engine that powers tools like 
GraphRAG, LightRAG, and Graphiti. 
...
✅ [**n8n_pipe**](https://openwebui.com/f/coleam/n8n_pipe/) - Cole Medin's N8N + OpenWebUI integration
...
**AI-Suite** includes a `start_services.py` script that handles the setup of Supabase and starting both Supabase and AI services.

Before running `start_services.py`, setup the Supabase environment variables using their [self-hosting guide
](https://supabase.com/docs/guides/self-hosting/docker#securing-your-services).

1. Make a copy of `.env.example` renamed to `.env` in the project directory.
   ```bash
...
3. Set the following environment variables if you wish to use your domain, otherwise leave commented:

   <details>
   <summary>Caddy environment variables</summary>
...
The script accepts the optional `--profile` argument to specify which GPU configuration to use.
...
2. Run Ollama on your Host for faster inference, and connect to that from the n8n instance:
   ```bash
...
If you're running OLLAMA in your Docker Host (not in Docker), modify the OLLAMA_HOST environment variable in the n8n service configuration. 
...
The `start_services.py` script supports a **private** (default) and **public** environment argument:
- **private:** you are deploying the stack in a safe environment, all AI-Suite ports are accessible
- **public:** the stack is deployed in a public environment, all AI-Suite ports except _80_ and _443_ are closed
...
> [!WARNING]
> ufw does not shield ports published by docker, because the iptables rules configured by Docker
> are analyzed before those configured by ufw. There is a solution to change this behavior, 
> but that is out of scope for this project. Just make sure that all traffic runs through the
> Caddy service via port _443_. Port _80_ should only be used to redirect to port _443_.

2. Run the `start_services.py` script with the environment argument **public** to indicate you 
   are going to run the package in a public environment. The script will make sure that all ports, 
   except for _80_ and _443_, are closed down, e.g.
...
> [!NOTE]
> If you are using a cloud machine without the "docker compose" command available by default
> such as a Ubuntu GPU instance on DigitalOcean, run these commands before running start_services.py:
...
to get started. Use credentials set in .env as required.

1. Open <http://localhost:5678/> in your browser to set up n8n. You’ll only
   have to do this once. You are NOT creating an account with n8n in the setup here,
   it is only a local account for your instance!

2. At the home workspace, set credentials for every service:

   - Ollama URL: http://ollama:11434 (n8n config), (browser http://localhost:11434)

   - Postgres account (through Supabase): use _db_, _username_, and _password_ from .env. 
     **IMPORTANT:** Host is 'db' since that is the name of the service running Supabase

   - Qdrant URL: http://qdrant:6333 (n8n config), (browser http://localhost:6333) 

   - Google Drive: Follow [this guide from n8n](https://docs.n8n.io/integrations/builtin/credentials/google/).

3. Open the workflow _V1 Local RAG AI Agent_ 
   <http://localhost:5678/workflow/vTN9y2dLXqTiDfPT>

   - Configure additional workflows:

     <details>
     <summary>RAG AI agent workflows</summary>

     _V2 Supabase RAG AI Agent_ 
     <http://localhost:5678/workflow/hrnPh6dXgIbGVzIk> 

     _V3 Local Agentic RAG AI Agent_ 
     <http://localhost:5678/workflow/RssROpqkXOm23GYL> 

     </details>

> [!NOTE]
> Don't use _localhost_ for the redirect URI, instead, use another domain. It will still work!
> Alternatively, you can set up [local file triggers](https://docs.n8n.io/integrations/builtin/core-nodes/n8n-nodes-base.localfiletrigger/).

4. Select **Test workflow** to confirm the workflow is properly configured.
...
6. Toggle the workflow as active.

7. Open <http://localhost:8080/> in your browser to set up Open WebUI.
   You’ll only have to do this once. You are NOT creating an account with Open WebUI in the 
   setup here, it is only a local account for your instance!

8. Go to Workspace -> Functions -> New Function -> Function Name + Function ID + Function Description 
   and copy then paste the code below to implement Cole Medin's n8n + OpenWebUI integration.
...
9. Copy the "Production" webhook URL from the workflow set in step 6.
...
With n8n, you have access to over 400 integrations and a suite of basic and advanced AI nodes such as:
...
> [!NOTE]
> The `start_services.py` script itself does not update containers - it only restarts them or pulls them 
> if you are downloading these containers for the first time. To get the latest versions, you must explicitly run
> the commands above.
