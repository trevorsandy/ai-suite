...
class Formatter(logging.Formatter):
    ...
    @staticmethod
    def prefix(color, bold=False, faint=False, italic=False, underline=False):
        """Return Select Graphic Rendition Control Sequence Introducer parameters"""
        # Resolve format conflict
        faint = False if bold and faint else faint
        # Set CSI parameters
        italic_fmt = ('3;{0}').format(color)
        underline_fmt = ('4;{0}').format(color)
        bold_fmt = ('1;{0}').format(color)
        bold_italic_fmt = ('3;{0}').format(bold_fmt)
        bold_underline_fmt = ('4;{0}').format(bold_fmt)
        bold_italic_underline_fmt = ('4;{0}').format(bold_italic_fmt)
        faint_fmt = ('2;{0}').format(color)
        faint_italic_fmt = ('2;{0}').format(italic_fmt)
        faint_underline_fmt = ('4;{0}').format(faint_fmt)
        faint_italic_underline_fmt = ('4;{0}').format(faint_italic_fmt)
        italic_underline_fmt = ('4;{0}').format(italic_fmt)
        # Return SGR parameters
        rendition = \
            bold_fmt if bold and not faint and not italic and not underline else \
            faint_fmt if faint and not bold and not italic and not underline else \
            italic_fmt if italic and not bold and not faint and not underline else \
            underline_fmt if underline and not bold and not italic and not faint else \
            bold_italic_fmt if bold and italic and not underline else \
            bold_underline_fmt if bold and underline and not italic else \
            bold_italic_underline_fmt if bold and italic and underline else \
            faint_italic_fmt if faint and italic and not underline else \
            faint_underline_fmt if faint and underline and not italic else \
            faint_italic_underline_fmt if faint and italic and underline else \
            italic_underline_fmt if underline and italic and not bold and not faint else \
            color
    ...
    def style(level:int | None=None, color:int | None=None, **kwargs):
        """Return Select Graphic Rendition style parameters
           kwargs (level): bold:bool, faint:bool, italic:bool, underline:bool, emoji:str
        ...
        """
        bold, faint, italic, underline, emoji = False, False, False, False, ''
        name_color, name_prefix, level_name_color, level_name_prefix = None, '', None, ''
        header_prefix, header, header_suffix, prefix, msg = '', '', '', '', ''
        for key, value in kwargs.items():
            if key in ['msg', 'message']:
                ...
                name_color = value
            elif key == 'level_name_prefix':
                level_name_prefix = value
            elif key == 'level_name_color':
                level_name_color = value
            elif key == 'header_prefix':
                header_prefix = value
            elif key == 'header':
                header = value
            elif key == 'prefix':
        ...
        if header:
            if not header_prefix:
                header_prefix = Formatter.prefix(color, True, faint, italic, underline)
            header_suffix = suffix + " " if not no_prefix else ""
        if not prefix and not no_prefix:
            prefix = Formatter.prefix(color, bold, faint, italic, underline)
        style_prefix = ("{0}{1}{2}{3}{4}{5}").format(
                        emoji, header_prefix, header, header_suffix, prefix, msg)
        style = {'prefix': style_prefix, 'suffix': suffix}
        if as_msg:
            style.update({'as_message': ("{0}{1}{2}").format(suffix, style_prefix, suffix)})
        if name:
            if not name_prefix:
                name_prefix = Formatter.prefix(name_color, bold, faint, True, underline)
        ...
        if level_name:
            if not level_name_prefix:
                ...
                level_name_prefix = Formatter.prefix(level_name_color, bold, faint, italic, underline)
...
def generate_searxng_secret_key():
    ...
    try:
        if system == "Windows":
            ...
            sed_cmd = ["sed", "-i", f"s|ultrasecretkey|{random_key}|g", settings_path]
            subprocess.run(sed_cmd, check=True)
        log.info("SearXNG secret key generated successfully.")
    except Exception as e:
    ...
    try:
        ...
        if is_first_run:
            ...
            for line in lines:
                ...
                f.write(line)
            message = "After the first run completes successfully, uncomment 'cap_drop:' " \
            ...
            for line in lines:
                ...
                if len(line_array) > 1 and env == line_array[0].strip():
                    ...
                    if len(var_array) > 1 and var == var_array[0].strip():
                        ...
                        mod_line = ''.join([env, '=\n'])
                    line = mod_line
                    message = f"Value for {env} removed..."
                    log.info("", extra=LSHF.style(header="Notice:", msg=message))
                f.write(line)
...
def container_is_running(container):
    ...
    try:
        ...
        if log.root.level == logging.DEBUG:
            ...
            message = " ".join([container, insert[0], insert[1]])
            log.debug("Container {}".format(message), extra=style)
        return running
    except subprocess.CalledProcessError:
        return False
...
def main():
    ...
    log.debug(f" - {llama_host_env}: {env_vars[llama_host_env]}", extra=debug_style)
    log.debug(f" - OPENAI_API_BASE_URL: {env_vars['OPENAI_API_BASE_URL']}", extra=debug_style)
